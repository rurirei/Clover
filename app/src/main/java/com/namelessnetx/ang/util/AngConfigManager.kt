...
import com.namelessnetx.ang.api.*
import com.namelessnetx.ang.api.V2rayConfig.Companion.DEFAULT_SECURITY
import com.namelessnetx.ang.api.V2rayConfig.Companion.TLS
import com.namelessnetx.ang.app
import com.namelessnetx.ang.app.ANG_CONFIG
import com.namelessnetx.ang.app.HTTPS_PROTOCOL
import com.namelessnetx.ang.app.HTTP_PROTOCOL
import com.namelessnetx.ang.util.MmkvManager.KEY_SELECTED_SERVER
...
object AngConfigManager {
    ...
    private fun copyLegacySettings(sharedPreferences: SharedPreferences) {
        ...
        listOf(
            ...
            app.PREF_ALLOW_INSECURE,
            app.PREF_PREFER_IPV6,
        ...
        settingsStorage?.encode(
            ...
            sharedPreferences.getBoolean(app.PREF_SNIFFING_ENABLED, true)
        )
        settingsStorage?.encode(
            app.PREF_PER_APP_PROXY_SET,
            sharedPreferences.getStringSet(app.PREF_PER_APP_PROXY_SET, setOf())
        )
    }
    ...
    private fun importConfig(
        ...
        try {
            ...
            val allowInsecure =
                false//settingsStorage?.decodeBool(app.PREF_ALLOW_INSECURE) ?: false
            if (str.startsWith(EConfigType.VMESS.protocolScheme)) {
                ...
                uri.rawQuery?.let { rawQuery ->
                    val queryParam = rawQuery.split("&")
                        .map {
                            it.split("=").let { (k, v) -> k to URLDecoder.decode(v, "utf-8")!! }
                        }
                        .toMap()
                    sni = queryParam["sni"] ?: ""
                }
        ...
        try {
            ...
            return config.configType.protocolScheme + when (config.configType) {
                ...
                EConfigType.VLESS -> {
                    ...
                    dicQuery["type"] =
                        if (streamSetting.network.isEmpty()) V2rayConfig.DEFAULT_NETWORK
