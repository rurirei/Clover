...
import com.namelessnetx.ang.model.SettingsViewModel
import com.namelessnetx.ang.service.V2RayServiceManager
import kotlinx.android.synthetic.main.toolbar_material.*
...
class SettingsActivity : BaseActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        private val perAppProxy by lazy { findPreference(app.PREF_PER_APP_PROXY) as CheckBoxPreference }
        private val localDns by lazy { findPreference(app.PREF_LOCAL_DNS_ENABLED) }
        private val fakeDns by lazy { findPreference(app.PREF_FAKE_DNS_ENABLED) }
        private val localDnsPort by lazy { findPreference(app.PREF_LOCAL_DNS_PORT) }
        private val vpnDns by lazy { findPreference(app.PREF_VPN_DNS) }
        private val sppedEnabled by lazy { findPreference(app.PREF_SPEED_ENABLED) as CheckBoxPreference }
        private val sniffingEnabled by lazy { findPreference(app.PREF_SNIFFING_ENABLED) as CheckBoxPreference }
        private val proxySharing by lazy { findPreference(app.PREF_PROXY_SHARING) as CheckBoxPreference }
        private val domainStrategy by lazy { findPreference(app.PREF_ROUTING_DOMAIN_STRATEGY) as ListPreference }
        private val routingMode by lazy { findPreference(app.PREF_ROUTING_MODE) as ListPreference }

        private val forwardIpv6 by lazy { findPreference(app.PREF_FORWARD_IPV6) as CheckBoxPreference }
        private val enableLocalDns by lazy { findPreference(app.PREF_LOCAL_DNS_ENABLED) as CheckBoxPreference }
        private val domesticDns by lazy { findPreference(app.PREF_DOMESTIC_DNS) as EditTextPreference }
        private val remoteDns by lazy { findPreference(app.PREF_REMOTE_DNS) as EditTextPreference }
...
//        val httpPort by lazy { findPreference(PREF_HTTP_PORT) as EditTextPreference }
        ...
        private val routingCustom: Preference by lazy { findPreference(app.PREF_ROUTING_CUSTOM) }
//        val donate: Preference by lazy { findPreference(PREF_DONATE) }
        //        val licenses: Preference by lazy { findPreference(PREF_LICENSES) }
//        val feedback: Preference by lazy { findPreference(PREF_FEEDBACK) }
//        val tgGroup: Preference by lazy { findPreference(PREF_TG_GROUP) }

        private val mode by lazy { findPreference(app.PREF_MODE) as ListPreference }
        ...
        override fun onCreatePreferences(bundle: Bundle?, s: String?) {
            ...
            domesticDns.setOnPreferenceChangeListener { _, any ->
                ...
                domesticDns.summary = if (nval == "") app.DNS_DIRECT else nval
                if (isRunning())
                    restartProxy()
                true
            }
            ...
            remoteDns.setOnPreferenceChangeListener { _, any ->
                ...
                val nval = any as String
                remoteDns.summary = if (nval == "") app.DNS_AGENT else nval
                if (isRunning())
                    restartProxy()
                true
            }
...
//            }
        ...
        override fun onStart() {
            ...
            updateMode(defaultSharedPreferences.getString(app.PREF_MODE, "VPN"))
            var remoteDnsString = defaultSharedPreferences.getString(app.PREF_REMOTE_DNS, "")
            domesticDns.summary = defaultSharedPreferences.getString(app.PREF_DOMESTIC_DNS, "")

            if (TextUtils.isEmpty(remoteDnsString)) {
                remoteDnsString = app.DNS_AGENT
            }
            if (domesticDns.summary == "") {
                domesticDns.summary = app.DNS_DIRECT
            }
            remoteDns.summary = remoteDnsString
            vpnDns?.summary = defaultSharedPreferences.getString(app.PREF_VPN_DNS, remoteDnsString)
...
//            lanconnPort.summary = defaultSharedPreferences.getString(PREF_HTTP_PORT, "")
        ...
        private fun updateMode(mode: String?) {
            ...
            perAppProxy.isChecked = PreferenceManager.getDefaultSharedPreferences(activity)
                .getBoolean(app.PREF_PER_APP_PROXY, false)
    ...
    fun onModeHelpClicked(view: View) {
        Utils.openUri(this, app.v2rayNGWikiMode)
    }
}
