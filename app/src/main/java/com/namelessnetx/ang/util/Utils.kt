...
object Utils {
    ...
    fun tryDecodeBase64(text: String): String? {
        try {
            ...
            Log.i(app.PACKAGE, "Parse base64 standard failed $e")
        }
        try {
            return Base64.decode(text, Base64.NO_WRAP.or(Base64.URL_SAFE)).toString(charset("UTF-8"))
        } catch (e: Exception) {
            Log.i(app.PACKAGE, "Parse base64 url safe failed $e")
        }
        return null
    }
    ...
    fun getRemoteDnsServers(): List<String> {
        val remoteDns = settingsStorage?.decodeString(app.PREF_REMOTE_DNS) ?: app.DNS_AGENT
        val ret = remoteDns.split(",").filter { isPureIpAddress(it) || it.startsWith("https") }
        if (ret.isEmpty()) {
            return listOf(app.DNS_AGENT)
        }
        return ret
    }

    fun getVpnDnsServers(): List<String> {
        val vpnDns = settingsStorage?.decodeString(app.PREF_VPN_DNS)
                ?: settingsStorage?.decodeString(app.PREF_REMOTE_DNS)
                ?: app.DNS_AGENT
    ...
    fun getDomesticDnsServers(): List<String> {
        val domesticDns = settingsStorage?.decodeString(app.PREF_DOMESTIC_DNS) ?: app.DNS_DIRECT
        val ret = domesticDns.split(",").filter { isPureIpAddress(it) || it.startsWith("https") }
        if (ret.isEmpty()) {
            return listOf(app.DNS_DIRECT)
        }
        return ret
    }
    ...
    fun stopVService(context: Context) {
        ...
        MessageUtil.sendMsg2Service(context, app.MSG_STATE_STOP, "")
    }
    ...
    fun testConnection(context: Context, port: Int): String {
        ...
        try {
            ...
            // network exception
            Log.d(app.PACKAGE,"testConnection IOException: "+Log.getStackTraceString(e))
            result = context.getString(R.string.connection_test_error, e.message)
        } catch (e: Exception) {
            // library exception, eg sumsung
            Log.d(app.PACKAGE,"testConnection Exception: "+Log.getStackTraceString(e))
            result = context.getString(R.string.connection_test_error, e.message)
        } finally {
            conn?.disconnect()
        }
    ...
    fun socketConnectTime(url: String, port: Int): Long {
        try {
            ...
            Log.d(app.PACKAGE, "socketConnectTime IOException: $e")
        } catch (e: Exception) {
            e.printStackTrace()
        }
        return -1
    ...
    fun getUrlContentWithCustomUserAgent(urlStr: String?): String {
        ...
        conn.setRequestProperty("Connection", "close")
        conn.setRequestProperty("User-agent", "NamelessnetX/${BuildConfig.VERSION_NAME}")
        url.userInfo?.let {
